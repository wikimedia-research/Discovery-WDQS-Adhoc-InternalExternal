...
SELECT
  date, traffic, referrer, agent_type, agent_type_plus, destination,
  -- aggregates:
  COUNT(1) AS requests,
  PERCENTILE_APPROX(ttfb, 0.5) AS median_ttfb,
  PERCENTILE_APPROX(ttfb, 0.95) AS upper95_ttfb,
  PERCENTILE_APPROX(ttfb, 0.99) AS upper99_ttfb,
  SUM(ttfb) AS total_ttfb
FROM wdqs_requests
